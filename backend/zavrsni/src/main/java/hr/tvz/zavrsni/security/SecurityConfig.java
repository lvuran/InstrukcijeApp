    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .authorizeHttpRequests(authorize -> {
                    authorize.requestMatchers("/", "/user", "/user/edit", "/location/all", "/teacher/all", "/student/subjects", "/teacher/lesson", "/teacher/alllessons", "/teacher/allrequests"
                    ).permitAll();
                    authorize.anyRequest().authenticated();
                })

                .oauth2Login(oauth2Login -> oauth2Login
                        .successHandler(successHandler())  // moj succes handler
